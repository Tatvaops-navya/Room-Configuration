/**
 * Local copy of the room watermark helpers so the Vite app can be deployed
 * independently from the root Next.js project.
 */
...
function logoUrlCandidates(): string[] {
  ...
  return [`${window.location.origin}${LOGO_PATH_SVG}`]
}
...
async function drawCornerLogoOnContext(
  ...
  for (const logoUrl of logoUrlCandidates()) {
    try {
      ...
      ctx.drawImage(logoImg, imgW - logoW - padding, imgH - logoH - padding, logoW, logoH)
      return
    } catch {
      continue
    }
